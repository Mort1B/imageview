...
enum ImagevError {
    ...
    #[error("An error occured while processing image")]
    ImageError(#[from] image::ImageError),
    #[error("An error occured while loading img")]
    IoError(#[from] std::io::Error),
    #[error("Unable to calculate maximim screen size on primary monitor")]
    NoPrimaryMonitor,
...
type Result<T> = std::result::Result<T, ImagevError>;
...
fn main() -> Result<()> {
    ...
    let primary_monitor = event_loop
        .primary_monitor()
        .ok_or(ImagevError::NoPrimaryMonitor)?;
